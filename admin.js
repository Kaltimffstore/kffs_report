...
function renderAdminCard(r, status) {
  ...
  if (status === 'pending') {
    actions = '<div class="admin-actions">' +
      '<button class="btn btn-success" onclick="updateStatus(\'' + r.id + '\', \'verified\')">✅ Terima</button>' +
      '<button class="btn btn-danger" onclick="updateStatus(\'' + r.id + '\', \'rejected\')">❌ Tolak</button>' +
      '<button class="btn btn-secondary" onclick="hapusLaporan(\'' + r.id + '\')" style="margin-left:auto;">🗑️ Hapus</button>' +
      '</div>';
  } else if (status === 'verified') {
    actions = '<div class="admin-actions">' +
      '<button class="btn btn-secondary" onclick="updateStatus(\'' + r.id + '\', \'rejected\')">❌ Batalkan</button>' +
      '<button class="btn btn-danger" onclick="hapusLaporan(\'' + r.id + '\')" style="margin-left:auto;">🗑️ Hapus</button>' +
      '</div>';
  } else {
    actions = '<div class="admin-actions">' +
      '<button class="btn btn-success" onclick="updateStatus(\'' + r.id + '\', \'verified\')">✅ Terima</button>' +
...
async function updateStatus(id, newStatus) {
  ...
  try {
    ...
    var result = await supabaseClient
      .from('laporan')
      .update({ status: newStatus, reviewed_at: new Date().toISOString(), reviewed_by: userResult.data.user.id })
      .eq('id', id);
